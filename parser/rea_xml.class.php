...
class REA_XML {
	...
	function parse_xml($xml_string) {
        ...
        if(is_array($all_properties) && count($all_properties) > 0) {
            foreach ($all_properties as $property) {
                ...
                // Get Property-Item Fields list
                $fieldList = $this->get_item_field_list($property_type);
                if( count($fieldList) )
                ...
                foreach($this->fields as $key => $field) {
                    ...
                    if(is_array($field)) {
                        ...
                        foreach($field as $subkey => $sub_field) {
                    
                            if($subkey != "attributecustom" && $subkey != "hotWaterService")
                            $prop[$key][$sub_field] = trim((string)$property->{$key}->{$sub_field});
                        else {
                            $arr = json_decode( json_encode($property->{$key}) , 1);
                            $i = 0;
                            foreach($arr as $attkey => $val ) {
                                $prop[$key][$i] =  $val;
                                $i++;
                            }
                         }
	...
	function parse_file($xml_full_path, $xml_file, $processed_dir=false, $failed_dir=false, $fieldList=array()) {
	...
	private function move_file($file, $file_full_path, $new_dir) {
		if(copy($file_full_path, $new_dir . "/$file".date("Ymd-").time())) {
	...
	private function feedback($string) {
        ...
        /**
         * Get Item related Field lists
         * @param string $propertyType Name of the property type (business, commercial, residential)
         * 
         * @return array Field list array
         */
        public function get_item_field_list($property_type) {
            
            if(!empty($property_type))
                switch ($property_type) {
                    
                    case "business";
                        global $business_fields;
                        return $business_fields;
                        break;
                    
                    case "commercial";
                        global $commercial_fields;
                        return $commercial_fields;
                        break;
                    
                    case "holidayRental";
                        global $holidayRental_fields;
                        return $holidayRental_fields;
                        break;
                    
                    case "rental";
                        global $rental_fields;
                        return $rental_fields;
                        break;
                    
                    case "residential";
                        global $residential_fields;
                        return $residential_fields;
                        break;
                    
                    case "land";
                        global $commercialLand_fields;
                        return $commercialLand_fields;
                        break;
